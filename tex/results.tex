...
% @AML: move next statement to the figure?
...
%Figure \ref{fig:architecture_locus_functionality} shows the functionality of each locus in the final dominant %organism's genome, using a representative sample from each treatment. 
%Here, we see that PLASTIC and STATIC treatments perform tasks in both environments. 
%The NONPLASTIC genome, however, only performs tasks in the [A/B] environment, meanwhile it a substantial number of vestigial loci in for the [other environment]

%Architecture analysis of each [replicate]'s final dominant lineage showed that NONPLASTIC populations experienced continuous genetic readaptation. 
%PLASTIC and STATIC lineages showed [...]
...
% @AML: needs trimming
In experimental Phase 2C (Section [XX]), we tested whether the lineages of adaptively plastic organisms accumulate more deleterious \code{poison} instructions than that of non-plastic lineages.
...
Additionally, we did not find compelling evidence that mutations that increased \code{poison} execution occurred as cryptic variation in PLASTIC lineages [supplement].
